...
<template>
  ...
  <section>
    <span
      v-if="isPending || (isFetching && !isFetchingNextPage)"
    ...
    <div
      v-else-if="items"
      v-for="(group, index) in items.pages"
      :key="index"
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-7 mt-14 justify-center mx-12"
    >
      <CardItem
